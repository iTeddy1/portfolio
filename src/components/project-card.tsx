...
export function ProjectCard({ project }: { project: Project }) {
  return (
    <div className="border-border shadow-shadow rounded-base bg-background flex flex-col border-2 p-4 sm:p-5">
      <AspectRatio
      ...
      <div className="text-foreground font-base mt-5">
        <div>
          <h2 className="font-heading text-xl sm:text-2xl">{project.name}</h2>

          <p className="mt-2">{project.description}</p>

          {project.role ? (
            <p className="mt-2 text-sm opacity-90">Role: {project.role}</p>
          ) : null}

          {project.stack && project.stack.length ? (
            <div className="mt-3 flex flex-wrap gap-2">
              {project.stack.map((tech, i) => (
                <Badge key={`${project.name}-tech-${i}`} variant="neutral">
                  {tech}
                </Badge>
              ))}
            </div>
